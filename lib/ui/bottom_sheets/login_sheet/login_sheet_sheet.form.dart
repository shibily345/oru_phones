...
final Map<String, TextEditingController> _LoginSheetTextEditingControllers = {};

final Map<String, FocusNode> _LoginSheetFocusNodes = {};

final Map<String, String? Function(String?)?> _LoginSheetTextValidations = {
  NumberInputValueKey: null,
...
mixin $LoginSheet {
  ...
  TextEditingController _getFormTextEditingController(
    ...
    if (_LoginSheetTextEditingControllers.containsKey(key)) {
      return _LoginSheetTextEditingControllers[key]!;
    }

    _LoginSheetTextEditingControllers[key] =
        TextEditingController(text: initialValue);
    return _LoginSheetTextEditingControllers[key]!;
  }

  FocusNode _getFormFocusNode(String key) {
    if (_LoginSheetFocusNodes.containsKey(key)) {
      return _LoginSheetFocusNodes[key]!;
    }
    _LoginSheetFocusNodes[key] = FocusNode();
    return _LoginSheetFocusNodes[key]!;
  }
  ...
  void disposeForm() {
    ...
    for (var controller in _LoginSheetTextEditingControllers.values) {
      controller.dispose();
    }
    for (var focusNode in _LoginSheetFocusNodes.values) {
      focusNode.dispose();
    }

    _LoginSheetTextEditingControllers.clear();
    _LoginSheetFocusNodes.clear();
  }
}
...
extension ValueProperties on FormStateHelper {
  ...
  set numberInputValue(String? value) {
    ...
    if (_LoginSheetTextEditingControllers.containsKey(NumberInputValueKey)) {
      _LoginSheetTextEditingControllers[NumberInputValueKey]?.text =
          value ?? '';
    }
  }
  ...
  set otpInputValue(String? value) {
    ...
    if (_LoginSheetTextEditingControllers.containsKey(OtpInputValueKey)) {
      _LoginSheetTextEditingControllers[OtpInputValueKey]?.text = value ?? '';
    }
  }
  ...
  set nameInputValue(String? value) {
    ...
    if (_LoginSheetTextEditingControllers.containsKey(NameInputValueKey)) {
      _LoginSheetTextEditingControllers[NameInputValueKey]?.text = value ?? '';
    }
  }
...
String? getValidationMessage(String key) {
  final validatorForKey = _LoginSheetTextValidations[key];
  if (validatorForKey == null) return null;

  String? validationMessageForKey = validatorForKey(
    _LoginSheetTextEditingControllers[key]!.text,
  );
